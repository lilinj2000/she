...
#define SL_EES_API_VERSION    "2.0.1.32"				///<  api版本号
...
{
  int               m_Result;							///< 返回0表示登录成功，非0表示登录失败，失败时，不会返回UserId   0-  登录成功    1-  用户名/密码错误   2-  用户存在配置问题，如账户列表为空等      3- 不允许重复登录       5 - 缺失客户端产品信息、MAC地址等必要信息, 6 - 服务器内部错误  7 - 未使用查询通道
